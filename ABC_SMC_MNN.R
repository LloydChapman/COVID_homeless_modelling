...
epsilon_S <- round(seq(sqrt(sum((2*D_S)^2)),sqrt(sum((D_S/2)^2)),length.out = 10))
...
sx_reporting_rate <- 55/106 #36/45 # 36 symptomatic cases in DPH data out of 45 in Liz's 4/27 data summary = 80% reporting rate
# Epsilon tolerance values for number of symptom onsets
epsilon_C <- round(seq(sqrt(sum((2*D_C)^2)),sqrt(sum((1.2*D_C)^2)),length.out = 10)) # this is ignored below if D_C=NULL
   	...
   	if(g==1){
    	# Sample from prior distribution
   	  # start_time <- Sys.time()
			...
			E0_star <- round(runif(1, min=lm.low[2], max=lm.upp[2])) #sample(lm.low[2]:lm.upp[2],1)
			T_sim_star <- round(runif(1, min=lm.low[3], max=lm.upp[3])) #sample(lm.low[3]:lm.upp[3],1)
			# end_time <- Sys.time()
		  ...
		  # start_time <- Sys.time()
			...
			R0_star <- par[1]
			beta_star <- calc_beta(R0_star,w,Present,p_s,Risk,h,alpha,mu_p,mu_sx)
			E0_star <- round(par[2]) #sample(lm.low[2]:lm.upp[2],1)
			T_sim_star <- round(par[3]) #sample(lm.low[3]:lm.upp[3],1)
			# end_time <- Sys.time()
			# print(end_time - start_time)
		}
			...
			for(j in 1:n){
			  ...
			  # start_time <- Sys.time()
				res <- COVID_homeless_model(N_res,N_staff,N_pop,T_sim_star,w,beta_star,epsilon,r_E,p_E,p_s,h,r_p,p_p,alpha,r_sx,p_sx,
				                            ...
				                            tmp,Number,Alive,Resident,Present,Risk,Age,e0ind,TrueState,DayTrueState,
  		  ...
  		  # Store results
  			...
  			if(g==1){
  			  ...
  			  # w2 <- sum(sapply(1:N, function(a) w.old[a]*mydtmvnorm(res.new[i,], mean=res.old[a,], sigma=sigma, lower=lm.low, upper=lm.upp)))
  				...
  				w2 <- sum(sapply(1:N, function(a) w.old[a]*dmvn(res.new[i,], mu=res.old[a,], sigma=sigma)))
  				# end_time <- Sys.time()
  				# print(end_time - start_time)
  			}
  			# print(w2)
