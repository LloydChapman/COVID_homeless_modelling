plot_calibration <- function(fnm1,fnm2,str,lm.low,lm.upp,end_date,D_S,D_T,D_C=NULL,testing_dates,sx_testing_dates,tol,ttl){
  ...
  #   }
  # }
  # p1 <- ggplot(res,aes(x=beta)) + geom_histogram(aes(y=..density..),binwidth = binwdths[1]) + theme_classic()
  # p2 <- ggplot(res,aes(x=E0,y=beta)) + stat_bin2d(aes(fill=..density..),binwidth=c(binwdths[2],binwdths[1]),show.legend = F) + theme_classic()
  # # ggplot(res,aes(x=E0,y=R0)) + stat_bin2d(aes(fill=..density..),show.legend = F,breaks=seq(lm.low[2]-0.5,lm.upp[2]+0.5,1)) + scale_x_continuous(breaks=lm.low[2]:lm.upp[2],limits = c(lm.low[2]-0.5,lm.upp[2]+0.5)) + scale_y_continuous(breaks=(lm.low[1]):(lm.upp[1])) + theme_classic()
  # p3 <- ggplot(res,aes(x=T,y=beta)) + stat_bin2d(aes(fill=..density..),binwidth=c(binwdths[3],binwdths[1]),show.legend = F) + theme_classic()
  # p4 <- ggplot(res,aes(x=beta,y=E0)) + stat_bin2d(aes(fill=..density..),binwidth=c(binwdths[1],binwdths[2]),show.legend = F) + theme_classic()
  # p5 <- ggplot(res,aes(x=E0)) + geom_histogram(aes(y=..density..),binwidth = binwdths[2],center = 0) + theme_classic()
  # p6 <- ggplot(res,aes(x=T,y=E0)) + stat_bin2d(aes(fill=..density..),binwidth=c(binwdths[3],binwdths[2]),show.legend = F) + theme_classic()
  # p7 <- ggplot(res,aes(x=beta,y=T)) + stat_bin2d(aes(fill=..density..),binwidth=c(binwdths[1],binwdths[3]),show.legend = F) + theme_classic()
  # p8 <- ggplot(res,aes(x=E0,y=T)) + stat_bin2d(aes(fill=..density..),binwidth=c(binwdths[2],binwdths[3]),show.legend = F) + theme_classic()
  # p9 <- ggplot(res,aes(x=T)) + geom_histogram(aes(y=..density..),binwidth = binwdths[3],center = 0) + theme_classic()
