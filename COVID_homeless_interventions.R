...
COVID_homeless_intervention_model<-function(N_res,N_staff,N_pop,T_sim,beta,epsilon,r_E,p_E,p_s,h,r_p,p_p,
  ...
  for (t in 2:T_sim) {
    ...
    # # Present[1:N_res] <- presence_update(Present[1:N_res],HxPCR[1:N_res],Risk[1:N_res]==1,sum(Risk[c(sx_indvdls_removed[sx_indvdls_removed<=N_res],PCRpos_removed[PCRpos_removed<=N_res])]==1) + sample(-5:5,1),0)
    # # Present[1:N_res] <- presence_update(Present[1:N_res],HxPCR[1:N_res],Risk[1:N_res]==2,sum(Risk[c(sx_indvdls_removed[sx_indvdls_removed<=N_res],PCRpos_removed[PCRpos_removed<=N_res])]==2) + sample(-5:5,1),0)
    # # Present[1:N_res] <- presence_update(Present[1:N_res],HxPCR[1:N_res],Risk[1:N_res]==3,sum(Risk[c(sx_indvdls_removed[sx_indvdls_removed<=N_res],PCRpos_removed[PCRpos_removed<=N_res])]==3) + sample(-5:5,1),0)
    # # Present[1:N_res] <- presence_update(Present[1:N_res],HxPCR[1:N_res],Risk[1:N_res]==4,sum(Risk[c(sx_indvdls_removed[sx_indvdls_removed<=N_res],PCRpos_removed[PCRpos_removed<=N_res])]==4) + sample(-5:5,1),0)
