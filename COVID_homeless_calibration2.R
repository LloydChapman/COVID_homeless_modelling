...
COVID_homeless_model<-function(N_res,N_staff,N_pop,T_sim,beta,epsilon,r_E,p_E,p_s,h,r_p,p_p,alpha,r_sx,p_sx,
  ...
  for (t in 2:T_sim) {
    ...
    # } else {
    #   Present[1:N_res] <- presence_update(Present[1:N_res],Risk[1:N_res]==1,CCMS_data$Total_Low_Risk[t-1],CCMS_data$Total_Low_Risk[t])
    #   Present[1:N_res] <- presence_update(Present[1:N_res],Risk[1:N_res]==2,CCMS_data$Hi_Risk_60_only[t-1],CCMS_data$Hi_Risk_60_only[t])
    #   Present[1:N_res] <- presence_update(Present[1:N_res],Risk[1:N_res]==3,CCMS_data$Hi_Risk_Dx_only[t-1],CCMS_data$Hi_Risk_Dx_only[t])
    #   Present[1:N_res] <- presence_update(Present[1:N_res],Risk[1:N_res]==4,CCMS_data$Hi_Risk_Both_Age_Dx[t-1],CCMS_data$Hi_Risk_Both_Age_Dx[t])
    # }
    ...
    if (t %in% sx_testing_days){ # Step 3.1: Selects for time steps where testing of cases with clinical symptoms takes place
      ...
      # } else {
      #   # No individuals with severe symptoms on testing day
      #   sx_indvdls_removed <- vector()
      # }
    } #else {
    #   sx_indvdls_removed <- vector()
...
# names(infections) <- seq(1,T_sim)
# barplot(infections,xlab = "Day", ylab = "No. new infections")
# plot(seq(1,T_sim), infections,type="l", xlab = "Day", ylab = "No. new infections")
# points(seq(1,T_sim),infections,pch=19)
# sim_data <- data.frame(cbind(day=1:T_sim,infections=infections))
